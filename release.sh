...
# deb 安装顺序: ocs2 → common → arms-ros2-control（standard）
# 真机 HI：src/arx-ros2-control 随发布包保留源码（不在 arms deb 内）
#
# 用法:
#   ./release.sh              # 交互式菜单
...
# 打包时保留的源码子模块（ocs2 / common / arms 由 deb 提供）
...
# 真机 HI 与描述见 src/arx-ros2-control、src/robot-descriptions-arx。
...
create_release_zip() {
  ...
  zip_name="lift2s_ws_${ts}_${arch}${git_tag}.zip"
...
do_package_release() {
  ...
  print_info "按 deb_versions.conf 准备 .deb_cache（版本匹配则复用，否则按 conf tag 下载）..."
  mkdir -p "${DEB_CACHE_DIR}"
...
发布打包 (--package / --package-no-git) 流程（在临时目录执行，不修改当前工作区）:
  ...
  3. 生成 ${DIST_DIR}/lift2s_ws_<时间>_<架构>[_nogit].zip 并删除临时目录
